...
pub enum KeyToolCommand {
    ...
    /// Given a string of data, sign with the fixed ephemeral key
    /// and output a zkLogin signature, default to devnet.
    ZkLoginSignPersonalMessage {
...
#[derive(Serialize)]
#[serde(rename_all = "camelCase")]
pub struct ZkLoginSignPersonalMessage {
    sig: String,
    bytes: String,
}
...
pub enum CommandOutput {
    ...
    ZkLoginSignPersonalMessage(ZkLoginSignPersonalMessage),
...
impl KeyToolCommand {
    pub async fn execute(self, keystore: &mut Keystore) -> Result<CommandOutput, anyhow::Error> {
        let cmd_result = Ok(match self {
            ...
            KeyToolCommand::ZkLoginSignPersonalMessage { data } => {
                ...
                CommandOutput::ZkLoginSignPersonalMessage(ZkLoginSignPersonalMessage {
                ...
                match GenericSignature::from_bytes(
                    ...
                    GenericSignature::ZkLoginAuthenticator(zk) => {
                        ...
                        for p in [
                            ...
                            let mut e = fetch_jwks(&p, &client).await?;
                            jwks.append(&mut e);
                        }
