...
use sui_package_resolver::store::LocalDBPackageStore;
    ...
    async fn process_checkpoint(&mut self, checkpoint_data: &CheckpointData) -> Result<()> {
        ...
        for checkpoint_transaction in checkpoint_transactions {
            for object in checkpoint_transaction.output_objects.iter() {
                self.package_cache.update_store(object).await?;
            }
...
impl ObjectHandler {
    pub fn new(store_path: &Path, rest_uri: &str) -> Self {
        let package_cache = Arc::new(PackageCache::with_store(Box::new(
            LocalDBPackageStore::new(&store_path.join("object"), rest_uri),
        )));
        ObjectHandler {
            objects: vec![],
            package_cache,
    ...
    async fn process_object(
        ...
        {
            let move_struct = get_move_struct(&tag, contents, &self.package_cache).await?;
            Some(move_struct)
        } else {
            None
        };
