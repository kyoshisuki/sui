...
impl EventHandler {
    pub fn new(store_path: &Path, rest_uri: &str) -> Self {
        let package_cache = PackageCache::with_store(Box::new(LocalDBPackageStore::new(
            &store_path.join("event"),
            rest_uri,
        )));
        EventHandler {
            events: vec![],
    ...
    async fn process_events(
        ...
        for (idx, event) in events.data.iter().enumerate() {
            ...
            let move_struct = get_move_struct(type_, contents, &self.package_cache).await?;
